...
function DocumentConversion(options, comments) {
    ...
    function getDefaultParagraphStyle(attr) {
        return htmlPaths.topLevelElement("p", attr);
    }
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            var el = style.to._elements[0];
            el.attributes = el.attributes || {};
            if (element.extraStyle){
                el.attributes.style = element.extraStyle || '';
            }
            return style.to;
        } else {
            if (element.styleId) {
            ...
            return getDefaultParagraphStyle(element.extraStyle ? {
                style: element.extraStyle
            } : {});
        }
    }
    ...
    function convertRun(run, messages, options) {
        ...
        var paths = [];
        if (run.extraStyle) {
            paths.push(htmlPaths.element('span', {style: run.extraStyle}, {fresh: false}));
        }
    ...
    function getDefaultTablePath(attr) {
        return htmlPaths.elements([
            htmlPaths.element("table", attr, {fresh: true})
        ]);
    }

    function convertTable(element, messages, options) {
        return findHtmlPath(element, getDefaultTablePath(element.extraStyle ? {
            style: element.extraStyle
        } : {})).wrap(function() {
    ...
    function convertTableRow(element, messages, options) {
        ...
        return [
            Html.freshElement("tr", element.extraStyle ? {
                style: element.extraStyle
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.extraStyle){
            attributes.style = element.extraStyle;
        }
